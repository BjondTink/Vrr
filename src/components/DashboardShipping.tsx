...
import { Save, RefreshCw, Truck, Loader2 } from "lucide-react";
...
export default function DashboardShipping() {
  const [shippingPage, setShippingPage] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [isDirty, setIsDirty] = useState(false);

  // Form Fields
  const [pageTitle, setPageTitle] = useState("Shipping & \n Transit.");
  const [pageSubtitle, setPageSubtitle] = useState("Studio Logistical Framework");
  const [pageBody, setPageBody] = useState("Every Vrr piece is hand-wrapped in tissue paper, loaded in custom organic cotton garment bags, and dispatched inside FSC-certified biodegradable containers directly from our atelier to minimize trace emissions.");
  const [pageImage, setPageImage] = useState("https://images.unsplash.com/photo-1566576912321-d58edd7a285f?auto=format&fit=crop&q=80&w=2670");
  ...
  useEffect(() => {
    ...
    const unsub = flexibleDb.subscribeToCollection("footerPages", (items) => {
      ...
      if (match) {
        setShippingPage(match);
        if (!isDirty) {
          setPageTitle(match.pageTitle !== undefined ? match.pageTitle : "Shipping & \n Transit.");
          setPageSubtitle(match.pageSubtitle !== undefined ? match.pageSubtitle : "Studio Logistical Framework");
          setPageBody(match.pageBody !== undefined ? match.pageBody : "Every Vrr piece is hand-wrapped in tissue paper, loaded in custom organic cotton garment bags, and dispatched inside FSC-certified biodegradable containers directly from our atelier to minimize trace emissions.");
          setPageImage(match.pageImage !== undefined ? match.pageImage : "https://images.unsplash.com/photo-1566576912321-d58edd7a285f?auto=format&fit=crop&q=80&w=2670");
        }
      } else {
        setShippingPage(null);
      }
      setLoading(false);
    });
    return unsub;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const currentPayload = shippingPage || {
        id: "shipping",
      ...
      await flexibleDb.saveDoc("footerPages", "shipping", updatedPayload);
      setIsDirty(false);
      alert("Shipping Page updated successfully!");
    } catch (err: any) {
      console.error("Save Shipping Page Err:", err);
      alert("Failed to save Shipping Page: " + err.message);
    } finally {
      setSaving(false);
    }
  };
  ...
  const discardChanges = () => {
    const baseTitle = shippingPage?.pageTitle !== undefined ? shippingPage.pageTitle : "Shipping & \n Transit.";
    const baseSubtitle = shippingPage?.pageSubtitle !== undefined ? shippingPage.pageSubtitle : "Studio Logistical Framework";
    const baseBody = shippingPage?.pageBody !== undefined ? shippingPage.pageBody : "Every Vrr piece is hand-wrapped in tissue paper, loaded in custom organic cotton garment bags, and dispatched inside FSC-certified biodegradable containers directly from our atelier to minimize trace emissions.";
    const baseImage = shippingPage?.pageImage !== undefined ? shippingPage.pageImage : "https://images.unsplash.com/photo-1566576912321-d58edd7a285f?auto=format&fit=crop&q=80&w=2670";
    ...
    <div className="p-4 lg:p-12 space-y-8 lg:space-y-12 h-screen overflow-y-auto pb-40">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white pb-6 border-b border-black/5 -mx-4 lg:-mx-12 lg:px-12 sticky top-0 z-20 p-4">
        <div>
          ...
          <h2 className="font-serif text-2xl tracking-tighter text-studio-black mt-1">Edit Shipping Page</h2>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 lg:gap-12">
        ...
        <form onSubmit={handleSave} className="xl:col-span-7 space-y-8 bg-[#f9f9f9] p-6 lg:p-10 rounded-2xl border border-black/5 font-sans">
          <div className="border-b border-black/5 pb-4">
            ...
            <p className="text-[10px] text-black/40 uppercase tracking-widest mt-1">Configure shipping narrative, delivery notes, and post details</p>
          </div>

          <div className="space-y-6">
            <SettingInput 
              label="Page Hero Subtitle (e.g. Studio Logistical Framework)" 
              value={pageSubtitle} 
            ...
            <SettingInput 
              label="Narrative body story / Postal and Delivery Notes (Supports multiple lines)" 
              textarea 
              rows={8}
              value={pageBody} 
            ...
            <div className="pt-2">
              <ImageUpload 
                label="Logistical Atmosphere / Shipping Image" 
        ...
        <div className="xl:col-span-5 space-y-4">
          <div className="border border-black/5 rounded-2xl bg-white p-4 shadow-sm">
            ...
            {/* Immersive Preview card replicating Shipping.tsx look */}
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-xs text-studio-black/70 leading-relaxed font-light whitespace-pre-line">
                    {pageBody || "Every Vrr piece is hand-wrapped..."}
                  </p>
                </div>
          ...
          <div className="p-4 bg-[#B39B84]/10 rounded-xl space-y-1.5 border border-[#B39B84]/10">
            <h4 className="text-[10px] uppercase tracking-widest font-extrabold text-[#B39B84]">Delivery Fee Rule Added:</h4>
            <p className="text-[10px] text-black/60 leading-relaxed">
              When users checkout, a standard <strong>300 Lek</strong> delivery shipping fee is automatically applied.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="sticky bottom-0 bg-white p-6 border-t border-black/5 flex justify-end z-20 shadow-2xl -mx-4 lg:-mx-12">
        ...
        >
          {saving ? (
            ...
            <>
              <Save size={14} />
              <span>Publish Shipping Page</span>
            </>
          )}
        </button>
      </div>
